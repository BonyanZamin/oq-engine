...
Module exports :class:`CampbellBozorgnia2003NSHMP2007`
                class:`CampbellBozorgnia2003NSHMP2007Vertical`.
"""
import numpy as np
...
def _compute_hanging_wall_effect(C, rjb, rrup, dip, mag, rake, vs30):
    ...
    """
    # frv, fth = 0, 0
    # if (dip > 45) and (22.5 <= rake <= 157.5):
    #     # normal faulting
    #     frv = 1
    # elif (dip <= 45) and (22.5 <= rake <= 157.5):
    #     # thrust faulting
    #     fth = 1
    # flag for reverse faulting
    frv = (dip > 45) & (22.5 <= rake) & (rake <= 157.5)
    ...
    # hw = np.array([(svfs + ssr + sfr) * (5 - r) / 5 if (r < 5 and dip <= 70) else 0.0
    #       for r in rjb])
...
class CampbellBozorgnia2003NSHMP2007Vertical(CampbellBozorgnia2003NSHMP2007):
